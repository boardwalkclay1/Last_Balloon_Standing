...
// ---------- BLE BRIDGE HOOKS ----------
// These call into ble-bridge.js (native BLE via Capacitor or similar)
...
// Called by native BLE when host broadcasts state (clients)
window.onBleStateReceived = function (jsonString) {
  try {
    const remote = JSON.parse(jsonString);
    gameState = remote;
...
// Called by native BLE when host receives an action (host)
window.onBleActionReceived = function (jsonString) {
  try {
    const action = JSON.parse(jsonString);
    applyAction(action);
...
function renderRound2(container) {
  ...
  if (me.id === spotlight.id) {
    ...
    // Render questions + answer inputs
    questionsByBalloon
      ...
      .forEach(qb => {
        ...
        saveBtn.onclick = () => {
          ...
          // Spotlight is host or client; send to host
          if (isHost) {
            applyAction(action);
            broadcastState();
            render();
...
function applyAction(action) {
  switch (action.type) {
    ...
    case 'QUESTION': {
      // Only accept questions from balloons still in
